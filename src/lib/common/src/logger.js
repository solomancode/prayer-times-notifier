...
function formatLine(log, type = 'default') {
    if (!log)
        return '';
    let line;
    switch (type) {
...
function log(message, meta, paramsData = []) {
    ...
    enableLogging && console.log(formatLine(formatDate(new Date())) +
        formatLine(message, 'info') +
        formatLine(meta.hint, 'info') +
        formatLine(c, 'counter') +
        fromattedData);
}
function extractParams(params, targetParams) {
    return targetParams ? targetParams.map((param) => {
